...
# Imported modules
import sys
from asyncio.streams import _DEFAULT_LIMIT, StreamReader, StreamWriter, open_connection
...
class Kelnet:
    ...
    async def open(self, host, port=0, timeout=DEFAULT_TIMEOUT):
        """Connect to a host.
        ...
        self.reader, self.writer = await open_connection(
            host=self.host, port=self.port, limit=DEFAULT_LIMIT
        )
    ...
    def msg(self, msg, *args):
        ...
        if self.debuglevel > 0:
            print("Kelnet(%s,%s):" % (self.host, self.port), end=" ")
            if args:
                print(msg % args)
            else:
                print(msg)
                ...
                if not self.iacseq:
                    ...
                    if cmd in (DO, DONT):
                        ...
                        if self.option_callback:
                            ...
                            self.writer.write(
                                IAC + DONT + opt
                            )  # telnetlib appears not to double IAC chars here
                            await self.writer.drain()
    ...
    async def expect(self, list, timeout=None):
